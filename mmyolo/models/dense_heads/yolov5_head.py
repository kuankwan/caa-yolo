...
from mmyolo.models.losses.da_loss import sigmoid_focal_loss
...
class YOLOv5Head(BaseDenseHead):
    ...
    # def da_loss(self, source_input: Tuple[Tensor],target_input: Tuple[Tensor]) -> dict:
    #     """Perform forward propagation and loss calculation of the detection
    #     head on the features of the upstream network.
    #
    #     Args:
    ...
    #     """
    #
    #     (s_cls_scores,s_bbox_preds,s_objectnesses) = self(source_input)
    #     (t_cls_scores,t_bbox_preds,t_objectnesses) = self(target_input)
    #     device = s_cls_scores[0].device
    #     loss_da = torch.zeros(1, device=device)
    #     batch_size, _, h, w = s_bbox_preds[0].shape
    #     # Fast version
    #     for i in range(self.num_levels):
    #         source_label = torch.full(s_cls_scores[i].shape,1,dtype=torch.float,device=device)
    #         target_label = torch.full(t_cls_scores[i].shape, 0, dtype=torch.float, device=device)
    #         loss_da += sigmoid_focal_loss(s_cls_scores[i],source_label) + sigmoid_focal_loss(t_cls_scores[i],target_label)
    #
    #     _, world_size = get_dist_info()
    #     losses = dict(
    #         loss_da=loss_da * batch_size * world_size,)
    #
    #     return losses
